...
>
> 🔴 **2026-08-29 又栽一次，恢复得太早了**：同一天下午派它做「本地先行」那张卡，**带着 `< /dev/null`
> 照样吊死**——跑到第 11 轮、日志停在「等模型回话」（`stream ... llm runtime selected`）之后
> **十分钟零日志、零输出增长、零文件改动**，内存还在缓慢变化所以不是进程死了，是流式接收吊住。
> 停掉换 Grok 重跑才拿到东西。**这是与输入口无关的第二种吊死，`< /dev/null` 治不了它。**
>
> **现行结论（三次事故后）**：opencode 可以派，但**只派短平快、可重跑的活**（补单测、机械替换、小修），
> **别派关键路径上的大卡**。派它就必须挂巡检：输出字节数和文件改动**连续 8–10 分钟双零**就判死、
> 停掉换人，别空等。判死前用 `ps -o rss=,etime=` 看一眼活体征，并去
> `~/.local/share/opencode/log/opencode.log` 按 `run=<id>` 看步数还涨不涨——**停在某一步的
> `stream` 之后再无日志 = 这一类吊死**。首选仍然是 Grok。
