...
> **同日查明一半**：「启动就吊住」这一类不是它的锅，是派活命令没关输入口——加上 `< /dev/null`
> （见下面调用模板第 ④ 条）后同一条命令从 5 分钟不动变成 18 秒干完，连跑三次稳定复现。当时据此把它恢复成正常工人。
>
> ⚠️ **2026-08-29 的「第三次吊死」是工头误判，已更正**：当天派它做「本地先行」那张卡，
> 巡检看到输出与文件改动连续十分钟双零，判死后换人。**事后查日志发现它根本没吊死**——
> 前 12 步里 11 步都在几秒到一分钟内走完，唯独**第 11 步到第 12 步花了 7 分 46 秒**，
> 之后它继续正常干活到第 12 步。**是模型某一次回话特别慢，不是挂了；工头在它刚缓过来时把它杀了。**
> 同样地，8/28 那次「跑了半小时没结论」的 b81 卡也不是吊死——日志显示它已走到第 32 步、
> 每十几秒推进一次，是在低效地探索。停掉它是合理的预算决定，但**「吊死」这个标签是错的**。
>
> **所以 opencode 至今只有一类确证的吊死**：启动阶段输入口没关（已修，见调用模板第 ④ 条，复现三次）。
> 「跑到一半吊死」目前**没有证据支持**。
>
> 🔴 **真正要改的是判死规则，不是 opencode 的评价**：
> - **输出字节数和文件改动为零，不等于死**。一次模型回话慢到 8 分钟是实测存在的，
>   而慢响应期间日志里什么都不会写——**「停在某一步的 `stream` 之后再无日志」既是吊死的样子，
>   也是慢响应的样子，光看这个分不开**。
> - 判死阈值**至少给到 15–20 分钟**（实测最慢单步 7m46s，留一倍余量），别用 8–10 分钟。
> - 判死前先看 `~/.local/share/opencode/log/opencode.log` 里 `run=<id>` 的**步数有没有涨过**；
>   涨过就说明它在干活，只是慢。再看 `ps -o rss=,etime=`。
> - 真要分辨「等在一个活着的连接上」还是「连接断了在空等」，用 `lsof -p <pid> | grep ESTABLISHED`。
> - **拿不准就继续等**：杀掉重跑的代价（整张卡从头来）远大于多等十分钟。
